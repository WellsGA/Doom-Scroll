        ...
        public void CreateTaskAssignerPanel()
        {
            ...
            foreach (GameData.PlayerInfo playerInfo in GameData.Instance.AllPlayers)
            {
                if (!playerInfo.IsDead)
                {
                    ...
                    //NEED TO FIX, changed original sr.color = Palette.PlayerColors[playerInfo.DefaultOutfit.ColorId];
                    //into just sr.color = new random color
                    sr.color = new Color32(198, 17, 17, byte.MaxValue);
                    PlayerButtons.Add(playerInfo.PlayerId, btn);
                    DoomScroll._log.LogInfo("Playercolor: " + Palette.GetColorName(playerInfo.DefaultOutfit.ColorId));
                    topLeftPos.x += 0.6f;
                }
            }
